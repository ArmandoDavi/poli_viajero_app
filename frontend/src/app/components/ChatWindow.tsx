...
interface ChatWindowProps {
  ...
  messages: Message[];
  isDarkMode: boolean;
  onSendMessage: (text: string) => void; // NUEVO
}
...
export function ChatWindow({
  ...
  // NUEVO: Función para manejar el envío
  const handleSend = () => {
    if (messageText.trim() !== '') {
    ...
    <div className={`flex flex-col h-full rounded-2xl overflow-hidden border ${
      ...
      <div className={`p-4 border-t ${isDarkMode ? 'border-[#2D0418]' : 'border-[#F0D0E0]'}`}>
        <div className={`flex items-center gap-2 rounded-full px-4 py-2 border ${
          isDarkMode ? 'bg-[#2D0418] border-[#3D0522]' : 'bg-[#FDE7F4] border-[#F0C8DF]'
        }`}>
          ...
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && handleSend()} // NUEVO: Enviar con Enter
            placeholder="Escribe un mensaje..."
            className={`flex-1 bg-transparent focus:outline-none ${
              isDarkMode ? 'text-[#E5E5E5] placeholder-[#A0A0A0]' : 'text-[#333333] placeholder-[#999999]'
            }`}
          />
          <button 
            onClick={handleSend} // NUEVO: Enviar al hacer clic
